...
- **Action** : Lancer un conteneur Redis nommé `redis` en utilisant le volume "redis_data" et le connecter au réseau "my_app".  
...
- **Action** : Lancer l'application Flask "myfirstapp" construite dans le TP précédent.  
  - dans le réseau "`my_app`"
  - en utilisant "`redis`" comme nom d'hôte pour Redis dans les variables d'environnement avec le paramètre `-e` et la variable nommée `REDIS_HOST`  
  - en exposant l'application sur le port 80
