...
- Lancez un build docker. La construction va être effectué directement dans le cluster. 
```
...
- Pendant que `skaffold dev --tail=false` tourne, on peut tester mettre volontairement port 3000 pour la livenessProbe et constater que Kubernetes redémarre les conteneurs frontend un certain nombre de fois avant d'abandonner.
