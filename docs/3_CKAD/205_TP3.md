...
- **Action** : Utiliser `kubectl exec` pour exécuter des commandes dans un Pod.  
  ...
  **Contraintes**
   ...
   - Commande à exécuter : `cat /etc/passwd`
...
- **Action** : Lancer un Pod avec un volume de type `PersistentVolume` pour écrire des données dans un fichier.  
  ...
  **Contraintes**
   - Nom du namespace : `mynamespace`
   - Nom du `PersistentVolumeClaim` : `pv-claim`.
   - Nom de la `storageClassName` du PVC : `longhorn`  
   - `AccessMode` du `PersistentVolumeClaim` : `ReadWriteMany`  
...
- **Action** : Ajouter des restrictions de sécurité à un Pod .  
  Un pod nommé `stress-pod`, utilise l'outil `stress` qui permet d'invoquer des consommations de CPU et de RAM arbitraires.    
  Le conteneur doit avoir un process lancé avec un user et un groupe déterminés via un `SecurityContext`.    
