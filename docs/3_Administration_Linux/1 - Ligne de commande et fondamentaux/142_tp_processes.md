...
- **7.2** - Créez le programme `fibonnaci_forever.sh` avec à l'intérieur le code :
...
fib 1000
```
Puis lancez `bash fibonnaci_forever.sh` dans un terminal.
...
- **7.7** - Lancez une session `byobu` (ou `screen`), puis dedans, lancer de nouveau le programme `fibonnaci_forever.sh`. Détachez la session, puis ré-attachez-là dans un autre terminal.
